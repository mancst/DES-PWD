...
function DES-PWE_config() {
	var x_config =  {
		// 迁移云管系统 fcgi - mt_slog_reportinfo 的调用地址, 注意开源版需要改为自己的地址
		report_url:'http://DES-PWE.com/cgi-bin/mt_slog_reportinfo',

		// 需要加载的插件列表, 插件和插件配置文件需和当前脚本放相同目录
		plugins_ary: ['monitor_website', ''],

		// 当前文件名, 如修改了文件名这里也需要相应修改
		js_self_name: 'dmt_DES-PWE.js', 
...
function DES-PWE(b_debug) {
	var x_config = DES-PWE_config();
	var xrk = DES-PWE_obj();
	...
	var xrk = {
		...
		init_log: function(log_config_id) {
			if(typeof log_config_id == 'undefined' || log_config_id == 0) {
				this.local_msg('DES-PWE - init failed, invalid log_config_id');
				return;
			}
		...
		report_to_server: function(delay) { 
			if(this.report_url == '') {
				this.local_msg('DES-PWE - report failed, request url not set !');
				return;
			}
		...
		local_msg: function(msg) {
			if(this.debug) {
				var pre = '' + new Date().getTime() + ' : DES-PWE - ' + msg;
		...
		is_DES-PWE_js : function(js_file) {
			var p_self = this;
		...
		_report_log: function(log_type, msg) {
			...
			{
				this.local_msg('DES-PWE - report failed, invalid log type(must - error|warn|reqerr|info|debug!');
				return;
			}
		...
		_report_to_server: function(delay) { 
			...
			if(typeof JSON === 'object' && JSON.stringify) {
				...
				p_self.local_msg('DES-PWE - report failed, not support JSON.stringify !');
			}
		},
			...
			try {
				...
				try {
					...
					try {
						...
						p_self.local_msg('DES-PWE - report failed, not support xmHttp request !');
						return false;
					}
				}
			};
