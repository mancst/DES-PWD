...
static int DealSetWarnChart()
{
	...
	const char *pvalue_type = hdf_get_value(stConfig.cgi->hdf, "Query.value_type", NULL);
	const char *pattr_name = hdf_get_value(stConfig.cgi->hdf, "Query.attr_name", NULL);
	if(pwarn_type==NULL || pvalue_type==NULL || pattr_name==NULL)
	{
		hdf_set_value(stConfig.cgi->hdf,"err.msg", CGI_REQERR);
		REQERR_LOG("invalid parameter %p|%p|%p", pwarn_type, pvalue_type, pattr_name);
		return SLOG_ERROR_LINE;
	}
	...
	hdf_set_value(stConfig.cgi->hdf, "config.value_type", pvalue_type);
