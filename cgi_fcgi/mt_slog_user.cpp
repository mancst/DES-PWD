...
static int DeleteUser()
{
	...
	if(qu.getval("login_type") != 0 || id==1) {
		// 管理员账号不允许删除
...
static int DealUserCenter()
{
	...
	if(user.has_bind_DES-PWE_uid()) {
		hdf_set_int_value(stConfig.cgi->hdf, "config.DES-PWE_account", user.bind_DES-PWE_uid());
		hdf_set_value(stConfig.cgi->hdf, "config.DES-PWE_key", user.bind_DES-PWE_key().substr(0, 6).c_str());
    }
...
static int DealInitChangeInfo()
{
	...
	/*
	   ...
	   *
	   */
	if(pf == NULL || iHasEmail < 0 || (strcmp(pf, "DES-PWE") && strcmp(pf, "name") 
...
static int DealTestDES-PWEWarn()
{
	if(IsDaemonDenyOp(stConfig))
		return 0;
...
static int DealUnBindDES-PWE()
{
	if(IsDaemonDenyOp(stConfig))
		return 0;

	int iDES-PWE_id = hdf_get_int_value(stConfig.cgi->hdf, "Query.DES-PWE_uid", -1);
	if(iDES-PWE_id < 0) {
		REQERR_LOG("invalid para:%d", iDES-PWE_id);
		stConfig.pErrMsg = CGI_REQERR;
		return SLOG_ERROR_LINE;
	}
	...
	if(user.bind_DES-PWE_uid() != 0) {
		...
		Query & qu = *stConfig.qu;
		sprintf(sBuf, 
			"update flogin_user set bind_DES-PWE_uid=0 where bind_DES-PWE_uid=%d and user_id=1",
			iDES-PWE_id);
		qu.execute(sBuf);
	}
...
static int DealBindDES-PWE()
{
	const char *pDES-PWE_name = hdf_get_value(stConfig.cgi->hdf, "Query.DES-PWE_uname", NULL);
    const char *pkey = hdf_get_value(stConfig.cgi->hdf, "Query.DES-PWE_bindkey", NULL);
	int iDES-PWE_id = hdf_get_int_value(stConfig.cgi->hdf, "Query.DES-PWE_uid", -1);
	const char *ppass = hdf_get_value(stConfig.cgi->hdf, "Query.pass", NULL);
	int iVerifyType = hdf_get_int_value(stConfig.cgi->hdf, "Query.verify_type", -1);
	if(!pkey || !pDES-PWE_name || iVerifyType != 1 || ppass==NULL || iDES-PWE_id < 0)
	{
		REQERR_LOG("invalid para(%p|%p[%s]|%p|%d|%d)", 
			pkey, pDES-PWE_name, (pDES-PWE_name != NULL ? pDES-PWE_name : ""),
			ppass, iVerifyType, iDES-PWE_id);
		stConfig.pErrMsg = CGI_REQERR;
		return SLOG_ERROR_LINE;
	}
	...
	if(ret == 0) {
		...
		snprintf(sBuf, sizeof(sBuf),
			"update flogin_user set bind_DES-PWE_uid=%d,bind_DES-PWE_key=\'%s\' where user_id=1",
            iDES-PWE_id, pkey); 
		if(!qu.execute(sBuf))
		{
			stConfig.pErrMsg = CGI_ERRDB_SERVER;
		...
		INFO_LOG("bind DES-PWE info uid:%d, uname:%s", iDES-PWE_id, pDES-PWE_name);

		user::UserSessionInfo & user = stConfig.stUser.pbSess;
		user.set_bind_DES-PWE_uid(iDES-PWE_id);
        user.set_bind_DES-PWE_key(pkey);
		FloginInfo *pUserInfo = stConfig.stUser.puser_info;
		SetUserSessionInfo(pUserInfo, user);
	}
...
int main(int argc, char **argv, char **envp)
{
	...
	while(FCGI_Accept() >= 0)
	{
		...
		else if(!strcmp(pAction, "bind_DES-PWE"))
			iRet = DealBindDES-PWE();
		else if(!strcmp(pAction, "unbind_DES-PWE"))
			iRet = DealUnBindDES-PWE();
		else if(!strcmp(pAction, "test_DES-PWE_sendwarn"))
			iRet = DealTestDES-PWEWarn();
		else {
			iRet = -1;
