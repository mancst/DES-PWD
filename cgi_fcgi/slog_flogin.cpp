...
static int GetBindDES-PWEUid()
{
	// 从内置管理员账号中获取云账号绑定情况
	Query qu(*stConfig.db);
	char sBuf[128] = {0};
	snprintf(sBuf, sizeof(sBuf), 
		"select bind_DES-PWE_uid from flogin_user where user_id=1 and xrk_status=0"); 
	int iBindDES-PWEUid = 0;
	if(qu.get_result(sBuf) && qu.fetch_row()) 
		iBindDES-PWEUid = qu.getval("bind_DES-PWE_uid");
	qu.free_result();
	return iBindDES-PWEUid;
}
...
static int CheckLogin(HDF *hdf)
{
	...
	if(stConfig.qu->get_result(sBuf) && stConfig.qu->fetch_row())
	{
		...
		{
			int iBindDES-PWEUid = stConfig.qu->getval("bind_DES-PWE_uid");
			g_iLoginType = stConfig.qu->getval("login_type");
			if(g_iLoginType == 1) {
				if(1 != stConfig.qu->getval("user_id"))
					iBindDES-PWEUid = GetBindDES-PWEUid();
			}
			...
			if(iBindDES-PWEUid != 0)
				localsess.set_bind_DES-PWE_uid(iBindDES-PWEUid);
