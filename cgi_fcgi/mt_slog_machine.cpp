	...
	if(!qu.get_result(ss.str().c_str()) || qu.num_rows() <= 0) {
		...
		while(qu.fetch_row()) {
			...
			if((bool)(plugin["b_self_detail"])) {
				ss.str("");
				ss << pszXrkUrl << "/plugin/" << (const char*)(plugin["plus_name"]) << ".html";
            	jp["desc_url"] = ss.str();
			}
			else if(plugin.HasValue("plus_url"))
            	jp["desc_url"] = (const char*)(plugin["plus_url"]);
			else
            	jp["desc_url"] = pszXrkUrl;
            js["plugins"].Add(jp);
            iPluginCount++;
        }
    }
...
static int DealShowMachineStatus()
{
    ...
    if(qu.get_result(ss.str().c_str()) && qu.num_rows() > 0) {
	    ...
	    for(int i=0; i < qu.num_rows() && qu.fetch_row() != NULL; i++)
        {
            ...
            info["plugin_version"] = qu.getstr("plugin_version");
...
static int DealSaveSysSrv(bool bIsAdd=false)
{
    ...
    char *pip_local = Str_Trim_Local((char*)ip);
    if(inet_addr(pip_local) == INADDR_NONE || iPort <= 0) {
        REQERR_LOG("invalid server - %s, %d", pip_local, iPort);
        return SLOG_ERROR_LINE;
    }
	...
	AddParameter(&ppara, "ip", pip_local, NULL);
	...
	DEBUG_LOG("%s mt_server success - ip:%s type:%d for:%s",
		bIsAdd ? "add" : "mod", pip_local, iType, pfor);
	return 0;
}
