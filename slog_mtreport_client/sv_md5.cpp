...
*/
 ...
 i = 0;
  ...
  MD5_memcpy((uint8_t *)&context->buffer[index], (uint8_t *)&input[i], inputLen-i);
}
...
void Md5HashBuffer(uint8_t *pOutBuffer, uint8_t *pInBuffer, uint32_t dwLength)
{
	...
	MD5Init(md5Info);
	MD5Update(md5Info, pInBuffer, dwLength);
	MD5Final(pOutBuffer, md5Info);
