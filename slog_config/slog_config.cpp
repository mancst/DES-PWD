...
typedef struct
{
    ...
    // 在某台服务器上配置为1 即可，如db 和 slog_config 混合部署则建议在db 部署的机器上配置为1
	...
	uint32_t dwLogConfigDelaySetTime;
	std::vector<uint32_t> stLogConfigDelay;
	...
	uint32_t dwLastWriteOtherInfoToDbTime;
	int32_t iOtherInfoToDbPerTime;

    int iHellePerTimeSec;
    int iAttrSendPerTimeSec;
    int iLogSendPerTimeSec;
...
int Init(const char *pFile = NULL)
{
	...
	if((iRet=LoadConfig(pConfFile,
		...
		"PLUGIN_INSTALL_CHECKSTR", CFG_STRING, stConfig.szPluginCheckStr, 
            "DES-PWE_inst_2020", MYSIZEOF(stConfig.szPluginCheckStr),
		...
		"MYSQL_PASS", CFG_STRING, stConfig.szPass, "mtreporT#875", MYSIZEOF(stConfig.szPass),
		...
		"DELETE_MONITOR_RECORDS", CFG_INT, &stConfig.iDeleteMonitorRecords, 1,
		...
		"WRITE_REANINFO_TO_DB_PER_TIME", CFG_INT, &stConfig.iOtherInfoToDbPerTime, 30,
		"AGENT_HELLO_PER_TIME_SEC", CFG_INT, &stConfig.iHellePerTimeSec, 5,
		"AGENT_ATTR_SEND_PER_TIME_SEC", CFG_INT, &stConfig.iAttrSendPerTimeSec, 15,
		"AGENT_LOG_SEND_PER_TIME_SEC", CFG_INT, &stConfig.iLogSendPerTimeSec, 10,
		(void*)NULL)) < 0)
	{   
    ...
    if(stConfig.iHellePerTimeSec < 5)
        stConfig.iHellePerTimeSec = 5;
    else if(stConfig.iHellePerTimeSec > 60)
        stConfig.iHellePerTimeSec = 60;

    if(stConfig.iAttrSendPerTimeSec < 5)
        stConfig.iAttrSendPerTimeSec = 5;
    else if(stConfig.iAttrSendPerTimeSec > 20)
        stConfig.iAttrSendPerTimeSec = 20;

    if(stConfig.iLogSendPerTimeSec < 5)
        stConfig.iLogSendPerTimeSec = 5;
    else if(stConfig.iLogSendPerTimeSec > 20)
        stConfig.iLogSendPerTimeSec = 20;
	...
	if(stConfig.iOtherInfoToDbPerTime < 5) {
		INFO_LOG("real info to db config time:%d to 5", stConfig.iOtherInfoToDbPerTime);
		stConfig.iOtherInfoToDbPerTime = 5;
	}
	else if(stConfig.iOtherInfoToDbPerTime > 60) {
		INFO_LOG("real info to db config time:%d to 60", stConfig.iOtherInfoToDbPerTime);
		stConfig.iOtherInfoToDbPerTime = 60;
	}
...
int32_t ReadTableModuleInfo(Database &db, int *pCustModuleId=NULL, uint32_t uid=0)
{
	...
	while(qu.fetch_row())
	{
        ...
        else if(iStatus == RECORD_STATUS_USE && iMatch < 0) 
        {
		    // 读表顺序依赖问题 - orderfix
            if(pCustModuleId == NULL) {
...
int32_t ReadTableWarnConfig(Database &db, uint32_t rid=0)
{
	...
	while(qu.fetch_row())
	{
		...
		// warn_type_value -- 提示对象 id 视图或者机器id, 两个的范围不能重叠
...
int32_t ReadTableConfigInfo(Database &db, uint32_t *pCustCfgId=NULL, uint32_t rid=0)
{
	...
	while(qu.fetch_row())
	{
		...
		else if(iStatus == RECORD_STATUS_USE && (iMatchApp < 0 || iMatchModule < 0))
		{
			...
			// 读表顺序依赖问题 - orderfix
			if(pCustCfgId == NULL) {
				stConfig.stLogConfigDelay.push_back(stLogConfig.dwCfgId);
                stConfig.dwLogConfigDelaySetTime = slog.m_stNow.tv_sec;
				WARN_LOG("orderfix - for log config:%u", stLogConfig.dwCfgId);
			}
			bReadAndSetOk = false;
		}
...
int32_t ReadTableFloginUser(Database &db, uint32_t uid=0)
{
	...
	while(qu.fetch_row())
	{
        ...
        if(iUserId == 1) {
	        MtSystemConfig *psysConfig = stConfig.psysConfig;
            if(psysConfig->iDES-PWEUid != qu.getval("bind_DES-PWE_uid")
                || !IsStrEqual(qu.getstr("bind_DES-PWE_key"), psysConfig->szDES-PWEKey))
            {
                psysConfig->iDES-PWEUid = qu.getval("bind_DES-PWE_uid");
                if(psysConfig->iDES-PWEUid != 0) {
                    strncpy(psysConfig->szDES-PWEKey, qu.getstr("bind_DES-PWE_key"), sizeof(psysConfig->szDES-PWEKey)-1);
                    psysConfig->szDES-PWEKey[sizeof(psysConfig->szDES-PWEKey)-1] = '\0';
                }
                psysConfig->dwDES-PWESetTime = slog.m_stNow.tv_sec;
                INFO_LOG("update DES-PWE cloud account, id:%d", psysConfig->iDES-PWEUid);
            }

            // other info
            ::user::SystemOtherInfo other;
            if(qu.getuval("other_info_seq") != psysConfig->stOtherInfoShm.dwOtherInfoSeq) {
		        qu.fetch_lengths();
        		unsigned long len = qu.getlength("other_info");
                const char *pval = qu.getstr("other_info");
                if(len <= 0 || len > sizeof(psysConfig->stOtherInfoShm.szOtherInfoBin)) 
                    WARN_LOG("get other info len:%lu invalid", len);
                else {
                    if(!other.ParseFromArray(pval, len)) {
                        WARN_LOG("ParseFromArray failed-%p-%lu", pval, len);
                    }
                    else {
                        if(len+128 >= (unsigned long)sizeof(psysConfig->stOtherInfoShm.szOtherInfoBin)) {
                            WARN_LOG("system szOtherInfoBin may need more space, use len:%lu", len);
                        }
                        memcpy(psysConfig->stOtherInfoShm.szOtherInfoBin, pval, len);
                        psysConfig->stOtherInfoShm.dwOtherInfoSeq = qu.getuval("other_info_seq");
                        psysConfig->stOtherInfoShm.iOtherInfoLen = (int)len;
                        INFO_LOG("get other info ok, len:%lu", len);
                    }
                }
            }
            else if(psysConfig->stOtherInfoShm.iOtherInfoLen > 0) {
                if(!other.ParseFromArray(psysConfig->stOtherInfoShm.szOtherInfoBin,
                    psysConfig->stOtherInfoShm.iOtherInfoLen))
                {
                    WARN_LOG("ParseFromArray failed-%d", psysConfig->stOtherInfoShm.iOtherInfoLen);
                }
                else {
                    if(!other.has_str_version_info()
                        || IsVersionUp(g_version_info.c_str(), other.str_version_info().c_str())) 
                    {
                        Query qu_tmp(db);
                        other.set_str_version_info(g_version_info);
                        slog.TryUpdateSysOtherInfo(qu_tmp, other);
                    }
                }
            }
            else {
		...
		for(int i=0; i < FLOGIN_SESSION_NODE_COUNT; i++)
		{
			...
			else if(stConfig.pshmLoginList->stLoginList[i].iUserId == iUserId) 
			{
				...
				else 
				{
                    ...
                    // 云账号绑定在内置管理员账号记录上
                    if(iUserId == 1) {
                        if(sess.bind_DES-PWE_uid() != qu.getval("bind_DES-PWE_uid")
                                || sess.bind_DES-PWE_key() != qu.getstr("bind_DES-PWE_key")) 
                        {
                            if(qu.getval("bind_DES-PWE_uid") == 0) {
                                sess.clear_bind_DES-PWE_uid();
                                sess.clear_bind_DES-PWE_key();
                            }
                            else {
                                sess.set_bind_DES-PWE_uid(qu.getval("bind_DES-PWE_uid"));
                                sess.set_bind_DES-PWE_key(qu.getstr("bind_DES-PWE_key"));
                            }
                            DEBUG_LOG("user bind info update uid:%d", sess.bind_DES-PWE_uid());
                            bUpSess = true;
                        }
                    }
					if(bUpSess)
...
int32_t ReadTableAttr(Database &db, uint32_t uid=0)
{
	...
	const char *ptmp = NULL;
	int iAttrType = 0, iStaticTime = 0, iValType = 0, iChartType = 0;
	MtSystemConfig *psysConfig = stConfig.psysConfig;
	SharedHashTable *pstHash = slog.GetWarnConfigInfoHash();
	...
	while(qu.fetch_row())
	{
        ...
        iValType = qu.getval("value_type");
        iChartType = qu.getval("chart_type");
		...
		{ 
			...
			else
			{
                ...
                pInfo->bValueType = iValType;
                pInfo->bChartType = iChartType;
		...
		else if(iStatus == RECORD_STATUS_USE && dwIsFind && pInfo->dwLastModTime != dwLastModTime)
		{
			...
			if(pInfo->iStaticTime != iStaticTime)
				pInfo->iStaticTime = iStaticTime;
            if(pInfo->bValueType != iValType)
                pInfo->bValueType = iValType;
            if(pInfo->bChartType != iChartType)
                pInfo->bChartType = iChartType;
...
int32_t ReadTablePluginMachineInfo(Database &db, uint32_t up_id=0)
{
	...
	while(qu.fetch_row() && qu.num_rows() > 0)
    {
        ...
        if(iStatus == 0 && qu.getval("install_proc") == EV_PREINSTALL_START) {
            // 一键部署事件
            if(qu.getval("inner_mode_install"))
        	    stConfig.pShmConfig->stSysCfg.stEvent[i].iEventType = EVENT_INNER_MODE_PREINSTALL_PLUGIN;
            else
	...
	while(qu.fetch_row())
	{
		...
		{ 
			...
			else
			{
                ...
                pInfo->dwClientIp = qu.getuval("client_ip");
                pInfo->dwGwIp = qu.getuval("gw_ip");
                pInfo->dwConnectIp = qu.getuval("conn_ip");
                pInfo->iTTL = qu.getval("cfg_ttl")-qu.getval("recv_ttl");
                pInfo->iResponseMs = qu.getval("server_response_time_ms");
		...
		else if(iStatus == RECORD_STATUS_USE && dwIsFind && dwLastModTime != pInfo->dwLastModTime)
		{
            ...
            if(pInfo->dwClientIp != qu.getuval("client_ip"))
                pInfo->dwClientIp = qu.getuval("client_ip");
            if(pInfo->dwGwIp != qu.getuval("gw_ip"))
                pInfo->dwGwIp = qu.getuval("gw_ip");
            if(pInfo->dwConnectIp != qu.getuval("conn_ip"))
                pInfo->dwConnectIp = qu.getuval("conn_ip");
            if(pInfo->iTTL != qu.getval("cfg_ttl")-qu.getval("recv_ttl"))
                pInfo->iTTL = qu.getval("cfg_ttl")-qu.getval("recv_ttl");
            if(pInfo->iResponseMs != qu.getval("server_response_time_ms"))
                pInfo->iResponseMs = qu.getval("server_response_time_ms");
			pInfo->dwLastModTime = dwLastModTime;
...
void InitSysConfig()
{
	...
	sys.wHelloPerTimeSec = stConfig.iHellePerTimeSec;
	sys.wCheckLogPerTimeSec = 20;
	sys.wCheckAppPerTimeSec = 22;
	sys.wCheckServerPerTimeSec = 24;
	sys.wCheckSysPerTimeSec = 26;
	sys.bAttrSendPerTimeSec = stConfig.iAttrSendPerTimeSec;
	sys.bLogSendPerTimeSec = stConfig.iLogSendPerTimeSec;
}
...
int CheckAllData()
{
	...
	// check log config
	if(stConfig.stLogConfigDelay.size() > 0) {
		INFO_LOG("orderfix - check delay log config, count:%d", (int)stConfig.stLogConfigDelay.size());
		if(dwCurTime >= stConfig.dwLogConfigDelaySetTime+stConfig.iCheckDataPerTime+DELAY_RECORD_CHECK_TIMEOUT) {
			DEBUG_LOG("clear log config orderfix, count:%u", (uint32_t)stConfig.stLogConfigDelay.size());
			stConfig.stLogConfigDelay.clear();
		}
		else {
			int iRemainCount = 0;
			for(unsigned i=0; i < stConfig.stLogConfigDelay.size(); i++)
			{
				uint32_t dwCfgId = stConfig.stLogConfigDelay[i];
				if(dwCfgId != 0) {
					if(ReadTableConfigInfo(*stConfig.pdb, &dwCfgId) == 0)
						stConfig.stLogConfigDelay[i] = 0;
					else
						iRemainCount++;
				}
			}
			if(iRemainCount <= 0)
				...
				DEBUG_LOG("orderfix log config has remain count:%d", iRemainCount);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	while(slog.Run())
	{
        // 每900-1000 ms check 一次
		usleep(900000+slog.m_iRand%100000);
		...
		{
			// 更新记录删除
			dwNextDelUpdateRecordsTime = dwCurTime+60+rand()%60;
			DelMonitorRecords(db, dwCurTime-60);
		}
	}
