...
public:
    ...
    // add by itc
    void SetRecvTTL(bool = true);
    int GetRecvTTL() { return m_recv_ttl; }
    struct timeval & GetRecvTS() { return m_recv_ts; }

    // 使用 map 实现可靠 udp, add by itc
...
protected:
	...
	void OnRead();
	int ReadIpOpts(char *ioBuf, int inBufSize, struct sockaddr *from, socklen_t fromlen);
