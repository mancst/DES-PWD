...
int UdpSocket::ReadIpOpts(char *ioBuf, int inBufSize, struct sockaddr *from, socklen_t fromlen)
{
	struct msghdr msg;
	struct iovec vec[1];
	union {
...
#define myCMSG_SPACE(l) (myALIGN(sizeof(struct cmsghdr)) + myALIGN(l))
	...
	if(n != -1 && msg.msg_controllen >= sizeof(struct cmsghdr) && !(msg.msg_flags & MSG_CTRUNC))
	{
		...
		{
			if (m_b_read_ts && cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_TIMESTAMP)
			{
				tv = (struct timeval *)CMSG_DATA(cmsg);
                if(tv)
                    memcpy(&m_recv_ts, tv, sizeof(struct timeval));
			}

            if(m_b_read_ttl && cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_TTL && cmsg->cmsg_len > 0)
            {
                m_recv_ttl = *(int *) CMSG_DATA(cmsg);
            }
		}
	}
...
void UdpSocket::OnRead()
{
	...
	if (IsIpv6())
	{
		...
		if (m_b_read_ts || m_b_read_ttl)
		{
            if(m_b_read_ts)
    			Utility::GetTime(&m_recv_ts);
            if(m_b_read_ttl)
                m_recv_ttl = -1;

			int n = ReadIpOpts(m_ibuf, m_ibufsz, (struct sockaddr *)&sa, sa_len);
			if (n > 0)
			{
...
void UdpSocket::SetRecvTTL(bool x)
{
    SetIpRecvTTL(x);
    m_b_read_ttl = x;
}
