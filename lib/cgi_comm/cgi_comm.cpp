...
void cgi_start_errlog(const char *pszFmt, ...)
{
	if(g_fpStartLog == NULL) {
		g_fpStartLog=fopen("/DES-PWE/cgi_start_error", "a+");
	}
	if(g_fpStartLog != NULL) {
		va_list ap;
		va_start(ap, pszFmt);
	...
	CGI *cgi, FloginList *pshmLoginList, const char *remote, uint32_t dwCurTime, CgiReqUserInfo *pinfo) 
{
	...
	// 看是否是迁移云管系统服务器发过来的请求
	if(g_strLoopIP == remote || slog.GetMachineInfoByIp((char*)remote) != NULL) {
		MtReport_Attr_Add(500, 1);
		DEBUG_LOG("get cgi request from server:%s, action:%s", remote, stConfig.pAction);
		if(CheckLoginEx_Local(s_sess, stConfig, cgi, pshmLoginList, remote, dwCurTime, pinfo) > 0)
            return 1;
	}
...
const char *GetPluginPreConfigItemDesc(const std::string &strCfgName) {
    ...
    if(strCfgName.find("XRK_PLUGIN_CONFIG_ID") != std::string::npos)
        return "插件日志配置ID, 为0表示不上报插件日志到迁移云管系统日志中心";
...
int InitFastCgiStart(CGIConfig &myConf)
{
	...
	if(LoadConfig(strGlobalConfgFile.c_str(), 
		...
		"DES-PWE_DEBUG", CFG_INT, &myConf.iDebugHtmlJs, 0, 
...
int InitFastCgi(CGIConfig &myCfg, const char *pszLogPath)
{
	...
	hdf_set_int_value(myCfg.cgi->hdf, "config.DES-PWE_debug", myCfg.iDebugHtmlJs);
...
int AfterCgiInit(CGIConfig &stConfig)
{
	...
	std::string strGlobalConfgFile;
	char *ptmp = NULL;
	if((ptmp=strrchr(stConfig.argv[0], '/')) != NULL) {
		strGlobalConfgFile.assign(stConfig.argv[0], (int)(ptmp-stConfig.argv[0])+1);
		strGlobalConfgFile += "xrk_fastcgi_global.conf";
    }
    else {
        ERR_LOG("invalid argument:%s, count:%d\n", stConfig.argv[0], stConfig.argc);
        return SLOG_ERROR_LINE;
    }
    char szDES-PWESiteAddr[256] = {0};
    LoadConfig(strGlobalConfgFile.c_str(),
        "DES-PWE_URL", CFG_STRING, szDES-PWESiteAddr, "http://DES-PWE.com", MYSIZEOF(szDES-PWESiteAddr),
        NULL);
	
    if(!IsStrEqual(stConfig.stUser.pSysInfo->stOtherInfoShm.szDES-PWEUrl, szDES-PWESiteAddr)) 
    {
        memset(stConfig.stUser.pSysInfo->stOtherInfoShm.szDES-PWEUrl, 
            0, sizeof(stConfig.stUser.pSysInfo->stOtherInfoShm.szDES-PWEUrl));
        strncpy(stConfig.stUser.pSysInfo->stOtherInfoShm.szDES-PWEUrl, szDES-PWESiteAddr, 
            sizeof(stConfig.stUser.pSysInfo->stOtherInfoShm.szDES-PWEUrl)-1);
    }
...
int BeforeCgiRequestInit(CGIConfig &stConfig)
{
	...
	slog.Run();
	stConfig.dwCurTime = time(NULL);
    stConfig.pErrMsg = NULL;
    stConfig.pMsgId = NULL;
	return 1;
}
...
int AfterCgiLogin(CGIConfig &stConfig)
{
    ...
    if(strncmp(psys->stOtherInfoShm.szDES-PWEUrl, "http://", 7) 
        && strncmp(psys->stOtherInfoShm.szDES-PWEUrl, "https://", 8))
	    hdf_set_valuef(stConfig.cgi->hdf, "config.DES-PWE_url=http://%s", psys->stOtherInfoShm.szDES-PWEUrl);
    else
	    hdf_set_value(stConfig.cgi->hdf, "config.DES-PWE_url", psys->stOtherInfoShm.szDES-PWEUrl);
	return 1;
}	
...
int SavePluginShowFile(Json &js, const std::string &strPath)
{
	...
	sscmd << "cd " << strPath << "; tar -zxf " << (const char*)(js["plus_name"]) << "_show.tar.gz;"
		<< " [ -f " << (const char*)(js["plus_name"]) << "_show/index_tp.html " 
        << " -o -f " << (const char*)(js["plus_name"]) << "/index_tp.html " << " ] && echo 0 || echo 1;";
	std::string strRet;
...
// 获取插件有监控点上报的机器
int GetMachineInfoForPluginAttr(CGIConfig &stConfig, Json &js, int32_t iPluginId, uint32_t dwStartUtcTime)
{
	Query & qu = *stConfig.qu;
	char sSqlBuf[256] = {0};
    sprintf(sSqlBuf, "select machine_id from mt_plugin_machine where open_plugin_id=%d and "
        " xrk_status=0 and install_proc=0 ", iPluginId);
