...
unsigned char * des::byte2bit(unsigned char byte[64] , unsigned char bit[8])
...
/*                     bit2byte                       */
/*----------------------------------------------------*/
unsigned char * des::bit2byte(unsigned char bit[8] , unsigned char byte[64])
...
/*                  change the key                  */
/*--------------------------------------------------*/
int des::keychange(unsigned char oldkey[8] , unsigned char newkey[16][8])
{
int i=0,j=0,k=0 ;
...
/*                  data encryption                 */
/*--------------------------------------------------*/
int des::endes(unsigned char m_bit[8] , unsigned char k_bit[8] , unsigned char e_bit[8])
{
...
/*                  data uncryption                 */
/*--------------------------------------------------*/
int des::undes(unsigned char m_bit[8] , unsigned char k_bit[8] , unsigned char e_bit[8])
...
/*                 S_replace                        */
/*--------------------------------------------------*/
int des::s_replace(unsigned char s_bit[8])
{
int p[32] = {   16,7,20,21,
		29,12,28,17,
		1,15,23,26,
...
void des::set_key(unsigned char *key)
{
	for(int i=0;i<8;i++)
	{
		this->key[i]=key[i];
...
string des::undes(string in) 
{
	string out="";
	unsigned char en_bit[8],un_bit[8];
	int n= in.length();
	if( n%8 != 0) 
	{
		//cout<<"error!"; 
...
string des::undes_hex(string in)
{
	string hex;	
	int len=in.length()/2;
	for(int i=0;i<len;i++)
	{
		if(in[2*i] >= 'A'&& in[2*i] <= 'F' )
			in[2*i] -= 'A' - 10;
		else
			in[2*i] -= '0';
		if(in[2*i+1] >= 'A'&& in[2*i+1] <= 'F' )
			in[2*i+1] -= 'A' - 10;
		else
			in[2*i+1] -= '0';
		hex+= char(in[2*i]*16+in[2*i+1]);
	}
	return undes(hex);
}
