...
static NEOERR *_parse_post_form (CGI *cgi)
{
  ...
  if (l == NULL){
	  // modify by itc -- firefox 
	  l = hdf_get_value (cgi->hdf, "CGI.Content-Length", NULL);
	  if(l == NULL)
		return STATUS_OK;
  }
  len = atoi (l);
...
NEOERR *cgi_output (CGI *cgi, STRING *str)
{
  ...
  output_f = hdf_get_value (cgi->hdf, "Config.DebugOutputFile", NULL);
  ...
  if (is_html)
  {
    ...
    if (do_timefooter)
    {
      ...
      cgi->time_end - cgi->time_start, use_deflate || use_gzip);
...
// 注意 cgi 必须要有读写 pszToFile 文件权限，否则写失败
int cgi_save_env_query_info(CGI *cgi, const char *pszToFile)
{
  char buf[256];
  int x = 0;
  ...
  {
    snprintf (buf, sizeof(buf), "CGI.%s", CGIVars[x].hdf_name);
    pvalue = hdf_get_value(cgi->hdf, buf, NULL); 
    if(pvalue != NULL)
        fprintf(fp, "%s=%s\n", CGIVars[x].env_name, pvalue);
    x++;
  }
