...
var DWZ = {
    ...
    statusCode: {
		ok: 200, okAndRefresh: 201, error: 300, timeout: 301, success: 0 
    } ,
    keys: {
    ...
    ajaxDone: function(json) {
        if (json[DWZ.keys.statusCode] == DWZ.statusCode.error ) {
			...
			// rock -- modify
			else if(typeof json.msg != 'undefined' && alertMsg) alertMsg.error(json.msg);
			else if(typeof json.msgid != 'undefined' && alertMsg) {
    ...
    init: function(pageFrag, options) {
        var op = $.extend({
            loginUrl: "login.html", loginTitle: null, ckeditorResources: null, ueditorResources: null,kindeditorResources:null, codemirrorResources : null, callback: null, debug: false, statusCode: {}
        }, options);
        this._set.loginUrl = op.loginUrl;
        this._set.loginTitle = op.loginTitle;
        this._set.debug = op.debug;
        this.editor.ckeditorResources = op.ckeditorResources;
        this.editor.ueditorResources = op.ueditorResources;
        this.editor.kindeditorResources = op.kindeditorResources;
        this.codemirror.resources = op.codemirrorResources;
        ...
        $.ajax({
            type: 'GET', url: pageFrag, dataType: 'html', timeout: 5000, cache: false, error: function(xhr) {
                ...
                $($.parseHTML(html, document)).each(function() {
            ...
            _doc.bind(DWZ.eventType.pageClear, function(event) {
                ...
                $("textarea.editor", $(box)).each(function() {
                    if('ckeditor'==$(this).attr('editorType')) {
                        ...
                        UE.delEditor[$(this).data("id")];
                    }
                });
                $("textarea.code", $(box)).each(function() {
                    delete DWZ.codemirror.instances[$(this).data("id")];
                });
            });
        }
    }
...
//            	debugger
        ...
        getSiblingsElemsH: function($container) {
        	...
        	height = headerH + pageBarH + panelBarH + formBarH + tabsHeaderH + contentTitleH;
        	return height;
        },
        /**
         * 获取元素高度
                ...
                if($(op.splitBar).is(':visible')){
                    ...
                    $(sbar).click(function() {
                        ...
                        function _hideBar() {
							...
							// rock add 
							$(op.toggleBut, sbar).trigger('click');
                        }
                        return false;
                    });
                }else{
...
( function($) {
    $.extend($.fn, {
        ...
        subTree: function(op, level) {
            return this.each(function() {
                $(">li", this).each(function() {
                    ...
                    $this.genTree({
                        root: op.root, icon: op.icon, ckbox: op.ckbox, exp: isLast ? op.options.lastExp: op.options.exp, coll: isLast ? op.options.lastColl: op.options.coll,
        ...
        genTree: function(options) {
            var op = $.extend({
                root: options.root, icon: options.icon, ckbox: options.ckbox, exp: "", coll: "", showSub: false, level: 0, options: null, isLast: false
            }, options);
            return this.each(function() {
                var node = $(this);
                var tree = $(">ul", node);
                ...
                if (tree.size() > 0 || node.attr(op.options.async)) {
                    ...
                    $(">div", node).prepend("<div class='" + ( ( op.showSub && !node.attr(op.options.async) ) ? op.coll: op.exp ) + "'></div>" + ( op.ckbox ? "<div class='ckbox " + checked + "'></div>": "" )
                            + ( op.icon ? "<div class='" + ( ( op.showSub  && !node.attr(op.options.async) )  ? op.options.folderColl: op.options.folderExp ) + "'></div>": "" ));
                    if (tree.size() > 0 ) {
                        op.showSub ? tree.show(): tree.hide();
                    }
                    $(">div>div:first,>div>div."+op.options.folderColl+",>div>div."+op.options.folderExp+",>div>a", node).click(function() {
                    ...
                    $(">div", node).prepend("<div class='node'></div>" + ( op.ckbox ? "<div class='ckbox " + checked + "'></div>": "" )
                            + ( op.icon ? "<div class='"+op.options.file+"'></div>": "" ));
                    if(op.icon ) {
...
function initEnv() {
	...
	// rock -add 
    var mybg = $("#my_background,#my_progressBar");
    mybg.hide().dblclick(function(){
        mybg.hide();
    });
    var needbg_timerid = 0;
    var ajax_start = 0;

    var ajaxbg = $("#aj_background,#aj_progressBar");
    ajaxbg.hide().dblclick(function(){
        ajaxbg.hide();
    });
    $(document).ajaxStart(function() {
		ajax_start = 1;
		needbg_timerid = setTimeout(function() {
			if(ajax_start)
				ajaxbg.show(); 
			}, 1500);
    }).ajaxStop(function() {
	...
	$.extend($.fn, {
		jPanel:function(options){
			...
			return this.each(function(){
				...
				// add by rock
				var OnToggle = $panel.attr("OnToggle")?$panel.attr("OnToggle"):"null";
...
function initUI(_box) {
    ...
    $('input[type="text"]').each(function() {
        ...
        var c_w = $(this).attr('cust_width');
        if(typeof c_w != 'undefined' && c_w != false) {
            $(this).css('width', c_w);
            return;
        }
        if(typeof s == 'undefined' || w == false)
            return;
        var w = s*8;
        $(this).css('width', ''+w+'px');
    });
...
( function($) {
    $.fn.extend({
        theme: function(options) {
            ...
            var _themeHref = op.themeBase + "/#theme#/style.css?v=16";
                    ...
                    type: "get", url: $a.attr("href"), callback: function(response) {
						...
						if(afirst != null) {
						    if(afirst.length <= 1) {
						        ...
						        var afirst = $("#sidebar").find("ul li ul li:first").find("a");
						        if(afirst != null) {
						            afirst.click();
						            $("#sidebar").find("ul li ul li:first").find("div:first").addClass('selected');
						        }
						    }
						} 
                    }
...
var alertMsg = {
    ...
    _open: function(type, msg, buttons) {
        ...
        $($.parseHTML(boxHtml, document)).appendTo("body").css({
...
var navTab = {
    ...
    _getScrollBarW: function() {
        return this.componentBox.width() - 171;
    } ,
...
( function($) {
    $.pdialog = {
		...
		// fix bug by itc@2020-08-26
        setCurrent: function(dialog_id) {
            this._current = $("body").data(dialog_id);
            return this._current;
        },
        reload: function(url, options) {
            var op = $.extend({
		...
		// rock add -本地打开一个层
		openLocal:function(dlgContentId, dlgid, title, options) {
...
//					$.pdialog.switchDialog(dialog);
				...
				jDContent.html( $("#"+dlgContentId).html() );
				jDContent.find("[layoutH]").layoutH(jDContent);
        ...
        open: function(url, dlgid, title, options) {
            ...
            // fix by itc btnLook 查找带回在对话框中使用bug
            if(typeof op.fk_callback == 'function')
                dialog.data('fk_callback', op.fk_callback);
        ...
        close: function(dialog) {
            ...
            // add by itc - fix bug @2020-08-25
            var fk_callback = dialog.data("fk_callback");
            var last_dialog_id = '';
            if(typeof fk_callback == 'function')
                last_dialog_id = fk_callback();
            ...
            if(last_dialog_id != '')
                this.setCurrent(last_dialog_id);
        },
        ...
        _resizeContent: function(dialog, width, height) {
            var content = $(".dialogContent", dialog);
            content.css({
                width: width + "px", height: height - $(".dialogHeader", dialog).outerHeight() - $(".dialogFooter", dialog).outerHeight()
            });
            content.find("[layoutH]").layoutH(content);
            $(".pageContent", dialog).css("width",  width + "px");
            $(window).trigger(DWZ.eventType.resizeGrid);
    ...
    $.taskBar = {
        ...
        init: function(options) {
            ...
            if (this._taskBar.size() == 0 ) {
                this._taskBar = $($.parseHTML(DWZ.frag["taskbar"], document)).appendTo($("body"));
 ...
 */
function noValidateCallback(form, callback, confirmMsg, bGlobal) {
	var $form = $(form);
	if(typeof(bGlobal) == "undefined")
		bGlobal = true;
	
	var ckfun =  callback || DWZ.ajaxDone;
 ...
 *            confirmMsg 提示确认信息
 */
function validateCallback(form, callback, confirmMsg, bGlobal) {
    var $form = $(form);
    ...
    $("textarea.code", $form).each(function() {
         $(this).val(DWZ.codemirror.instances[$(this).data("id")].getValue());
         if('true'==$(this).attr('escape')){
             $(this).val(html2Escape($(this).val()));
         }
    });
    ...
    var _submitFn = function() {
        ...
        $.ajax({
            type: form.method || 'POST', url: $form.attr("action"), 
			data: $form.serializeArray(), dataType: "json", 
			cache: false, success: callback || DWZ.ajaxDone ,
			global: bGlobal || true,
            error: DWZ.ajaxError
        });
    }
...
function iframeCallback(form, callback) {
    ...
    if ($iframe.size() == 0 ) {
        $iframe = $($.parseHTML("<iframe id='callbackframe' name='callbackframe' src='about:blank' style='display:none'></iframe>", document)).appendTo("body");
    }
    if (!form.ajax ) {
...
function dwzPageBreak(options) {
    ...
    if (op.rel ) {
		...
		// add by itc --- 2016-11-21 js 主动调用
		if(typeof op.targetType == 'function') {
			op.targetType(op.data);
			return;
		}
...
function ajaxTodoPost(url, data, callback){
	var $callback = callback || navTabAjaxDone;
	...
	$.ajax({
		...
		dataType: 'json',
		cache: false,
		global: data.global != undefined ? data.global : true,
		success: $callback,
		error: DWZ.ajaxError,
		timeout: function(){
...
function ajaxTodoType(url, callback, reqType, dateType){
	var $callback = callback || navTabAjaxDone;
	if (! $.isFunction($callback)) $callback = eval('(' + callback + ')');
	$.ajax({
		type:reqType || 'GET',
		url:url,
		dataType:dateType || "json",
		cache: false,
		global: false,
		success: $callback,
		error: DWZ.ajaxError
...
mask: true, width: $this.attr('width') || 820, height: $this.attr('height') || 500, maxable: eval($this.attr("maxable") || "true") , resizable: eval($this.attr("resizable") || "true"), fk_callback: eval($this.attr('fk_callback'))
                };
        ...
        itemDetail: function() {
            return this.each(function() {
                ...
                if (addButTxt ) {
                    ...
                    if(batchUploadUrl){
                        var $batchButton = $('<label><a class="btnAttach" lookupGroup="" href="'+batchUploadUrl+'" width="560" height="300" ></a></label>').initUI().insertBefore($table).find('a');
                    ...
                    $addBut.click(function() {
                        ...
                        for (var i = 0; i < rowNum; i++) {
                            var $tr = $($.parseHTML(trTm, document));
                ...
                switch (field.type) {
                    ...
                    case 'attach':
                        html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix + '"/>' + '<input type="text" name="' + field.name + '" size="'
                                ...
                                + '" ' + suffixFrag + ' lookupPk="' + field.lookupPk + '" width="560" height="300" ' + attrFrag + '>'+field.title+'</a>';
                        break;
                    case 'enum':
                        $.ajax({
    ...
    $.fn.datepicker = function(opts) {
        ...
        return this.each(function() {
            ...
            $this.click(function(event) {
                ...
                var iTop = offset.top + this.offsetHeight;
                $($.parseHTML(DWZ.frag['calendarFrag'], document)).appendTo("body").css({
                    left: offset.left + 'px', top: iTop + 'px'
        ...
        combox: function() {
            ...
            return this.each(function(i) {
                ...
                if( "undefined" !== typeof value ){
                    label = $("option[value=" + value.replaceAll('\\.','\\.') + "]", $this).text();
                }
