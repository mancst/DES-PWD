...
/*
document.getElementById('show_input').addEventListener('keypress', ({ keyCode, target }) => {
  if (keyCode === 13) {
    clearTimeout(timer);
...
shape.init();
shape.print('基于服务的云平台应用程序监控分析系统');
var g_show_txt = 0;
function dlLoopShowInfo() {
	shape.clear();
	window.setTimeout(function() {
		...
		window.setTimeout(function() { dlLoopShowInfo(); }, 3000);
	}, 1000);
}
window.setTimeout(function() { dlLoopShowInfo(); }, 3000);
